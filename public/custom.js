...
let FingerprintSdkTest = (function () {
    ...
    FingerprintSdkTest.prototype.startCapture = function () {
        ...
        this.sdk.startAcquisition(currentFormat, "").then(function (data) {
...
function readyForEnroll(){
    return ((document.getElementById("enrollReaderSelect").value !== "Select Fingerprint Reader"));
}
...
function flipVerticalBase64(imageBase64, callback) {
    ...
    $('#picture').attr('src','data:image/png;base64,' + imageBase64);
    // console.log(img);
    // $('#fff').html(img)
    // document.getElementById("fff").innerHTML = img;
//    console.log(img);
...
function storeSample(sample){
    ...
    let nextElementID = getNextNotEnrolledID();
    const token = samples[0];
  
    var temp = Base64.decode(token);
    // console.log(token);
    var b64UrlTo64 = Fingerprint.b64UrlTo64(samples[0]);
    var b64To64Url = Fingerprint.b64To64Url(samples[0]);
    var b64UrlToUtf8 = Fingerprint.b64UrlToUtf8(samples[0]);
    var strToB64Url = Fingerprint.strToB64Url(samples[0]);

    // console.log(b64UrlTo64);
    // console.log(b64To64Url);
    // console.log(b64UrlToUtf8);
    // console.log(strToB64Url);
    // console.log(Base64.decode(token));
    flipVerticalBase64(b64UrlTo64, (imageFlipped)=>{
    //     // var imagSrc = 'data:image;base64,' + imageFlipped;
    //     // fingerprint.src = imagSrc;
    //     // console.log(imagSrc);
    //     // document.getElementById("fingerprint").src = imagSrc;
    });
    if(nextElementID.startsWith("index") || nextElementID.startsWith("verification")){
